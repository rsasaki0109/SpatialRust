...
|------|------|
| 実行日 | 2026-06-12 |
| テスト | `cargo test -p spatialrust --features mvp --test mvp_pipeline` → **20 passed**（`pipeline-mvp-gpu` 追加時 **22 passed**） |
...
| IO 検証 | PCD/LAS/COPC roundtrip、xyzirgb 複合、**50k scan-like LAS/COPC ファイル MVP（Epic 41）**、**COPC CLI `--resolution` / `--bounds`+`--resolution`（Epic 43–44）** |
