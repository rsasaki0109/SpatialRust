...
| IO 検証 | PCD roundtrip、LAS roundtrip、COPC roundtrip、**COPC bounds query → MVP**（`mvp_copc_query_pipeline`） |
...
1. MVP CLI に COPC query オプション（`--bounds`）
2. approximate_first モード向け別閾値の検討
3. 1M 点 filter ベンチ再計測
