...
///
/// Uses the same radius graph as the former WGSL label-propagation kernel but
/// resolves components in one CPU pass with path compression — much faster than
/// Jacobi iteration on large scans. Returns one root index per point (minimum
...
pub fn euclidean_cluster_roots_gpu(
    ...
    euclidean_cluster_roots_grid(x, y, z, cluster_tolerance)
}

/// Grid-based union-find clustering (CPU, zero-copy over slices).
pub fn euclidean_cluster_roots_grid(
    x: &[f32],
    y: &[f32],
    z: &[f32],
    cluster_tolerance: f32,
...
#[cfg(test)]
mod tests {
    use super::euclidean_cluster_roots_grid;
    ...
    fn long_chain_is_one_component() {
        ...
        let roots = euclidean_cluster_roots_grid(&x, &y, &z, 1.5).unwrap();
