...
@group(0) @binding(5) var<storage, read> values_c2: array<f32>;
@group(0) @binding(6) var<storage, read> values_c3: array<f32>;
@group(0) @binding(7) var<storage, read_write> output_c0: array<f32>;
@group(0) @binding(8) var<storage, read_write> output_c1: array<f32>;
@group(0) @binding(9) var<storage, read_write> output_c2: array<f32>;
@group(0) @binding(10) var<storage, read_write> output_c3: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if params.channel_count >= 3u {
        output_c2[cell] = values_c2[index];
    }
    if params.channel_count >= 4u {
        output_c3[cell] = values_c3[index];
    }
}
