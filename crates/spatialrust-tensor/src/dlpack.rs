...
#[repr(C)]
struct RawManagedTensorLegacy {
    dl_tensor: RawTensor,
    manager_ctx: *mut c_void,
    deleter: Option<unsafe extern "C" fn(*mut RawManagedTensorLegacy)>,
}
...
pub struct DlpackLegacyExport {
    raw: NonNull<RawManagedTensorLegacy>,
}
...
/// # Safety
///
/// `raw` must carry exclusive deleter responsibility for a live
/// legacy `DLManagedTensor*`. It must not be used after this call.
pub unsafe fn release_dlpack_legacy_raw(raw: *mut c_void) {
    // SAFETY: forwarded from the public ownership contract above.
    unsafe { call_legacy_deleter(raw.cast()) };
}
...
impl DlpackLegacyExport {
    ...
    pub fn from_tensor(tensor: &TensorBuffer) -> Result<Self, DlpackError> {
        let (context, tensor) = build_export(tensor)?;
        let raw = Box::new(RawManagedTensorLegacy {
            dl_tensor: tensor,
            manager_ctx: Box::into_raw(context).cast(),
            deleter: Some(delete_legacy_export),
        });
        Ok(Self { raw: NonNull::from(Box::leak(raw)) })
    }
...
impl Drop for DlpackLegacyExport {
    fn drop(&mut self) {
        // SAFETY: this export uniquely owns deleter responsibility.
        unsafe { call_legacy_deleter(self.raw.as_ptr()) };
    }
}
...
#[derive(Debug)]
enum ImportedRaw {
    Versioned(NonNull<RawManagedTensorVersioned>),
    Legacy(NonNull<RawManagedTensorLegacy>),
}
...
impl DlpackImport {
    ...
    pub unsafe fn from_raw(raw: *mut c_void) -> Result<Self, DlpackError> {
        ...
        let raw = ImportedRaw::Versioned(guard.disarm());
        Ok(Self {
            raw,
            descriptor,
    ...
    pub unsafe fn from_legacy_raw(raw: *mut c_void) -> Result<Self, DlpackError> {
        let raw = NonNull::new(raw.cast::<RawManagedTensorLegacy>())
            .ok_or(DlpackError::NullManagedTensor)?;
        let guard = IncomingLegacyGuard { raw: Some(raw) };
        // SAFETY: the caller promises a live legacy managed tensor.
        let managed = unsafe { raw.as_ref() };
        let (descriptor, allocation_len, data) = validate_tensor(&managed.dl_tensor)?;
        let raw = ImportedRaw::Legacy(guard.disarm());
        Ok(Self { raw, descriptor, allocation_len, data, version: (0, 0), flags: 0 })
    }
...
impl Drop for DlpackImport {
    fn drop(&mut self) {
        // SAFETY: this owner received exclusive deleter responsibility.
        unsafe {
            match self.raw {
                ImportedRaw::Versioned(raw) => call_deleter(raw.as_ptr()),
                ImportedRaw::Legacy(raw) => call_legacy_deleter(raw.as_ptr()),
            }
        };
    }
}
...
struct IncomingLegacyGuard {
    raw: Option<NonNull<RawManagedTensorLegacy>>,
}

impl IncomingLegacyGuard {
    fn disarm(mut self) -> NonNull<RawManagedTensorLegacy> {
        self.raw.take().expect("incoming pointer is present")
    }
}

impl Drop for IncomingLegacyGuard {
    fn drop(&mut self) {
        if let Some(raw) = self.raw {
            // SAFETY: the guard owns the transferred pointer on error paths.
            unsafe { call_legacy_deleter(raw.as_ptr()) };
        }
    }
}
...
unsafe extern "C" fn delete_legacy_export(raw: *mut RawManagedTensorLegacy) {
    if raw.is_null() {
        return;
    }
    // SAFETY: installed only for allocations built by `DlpackLegacyExport`.
    let managed = unsafe { Box::from_raw(raw) };
    if !managed.manager_ctx.is_null() {
        // SAFETY: manager_ctx was created with Box::into_raw for ExportContext.
        drop(unsafe { Box::from_raw(managed.manager_ctx.cast::<ExportContext>()) });
...
unsafe fn call_legacy_deleter(raw: *mut RawManagedTensorLegacy) {
    if raw.is_null() {
        return;
    }
    // SAFETY: caller owns a live legacy managed-tensor pointer.
    if let Some(deleter) = unsafe { (*raw).deleter } {
        // SAFETY: deleter belongs to this exact managed tensor.
        unsafe { deleter(raw) };
    }
...
fn build_export(tensor: &TensorBuffer) -> Result<(Box<ExportContext>, RawTensor), DlpackError> {
    let descriptor = tensor.descriptor();
    if !descriptor.device().is_host_accessible() {
        return Err(TensorError::DeviceNotHostAccessible(descriptor.device()).into());
    }
    ...
    let allocation = tensor.shared_allocation();
    let data =
        if allocation.is_empty() { ptr::null_mut() } else { allocation.as_ptr().cast_mut().cast() };
    let shape_ptr = if shape.is_empty() { ptr::null_mut() } else { shape.as_ptr().cast_mut() };
    let strides_ptr =
        if strides.is_empty() { ptr::null_mut() } else { strides.as_ptr().cast_mut() };
    let context =
...
fn validate_tensor(
    tensor: &RawTensor,
) -> Result<(TensorDescriptor, usize, *const u8), DlpackError> {
    if tensor.ndim < 0 || tensor.ndim as usize > MAX_RANK {
        return Err(DlpackError::InvalidRank(tensor.ndim));
    }
    let rank = tensor.ndim as usize;
    ...
    let shape_values = if rank == 0 {
        ...
        // SAFETY: the producer contract supplies `ndim` readable shape entries.
        unsafe { slice::from_raw_parts(tensor.shape, rank) }
    };
    let shape = shape_values
        .iter()
    ...
    let strides = if tensor.strides.is_null() {
        ...
        // SAFETY: the producer contract supplies `ndim` readable stride entries.
        let values = unsafe { slice::from_raw_parts(tensor.strides, rank) };
        Some(
            values
                .iter()
    ...
    let byte_offset =
        usize::try_from(tensor.byte_offset).map_err(|_| DlpackError::IntegerConversion)?;
    let descriptor = match strides {
        Some(strides) => TensorDescriptor::try_strided(dtype, shape, strides, byte_offset, device)?,
        None => {
            let mut descriptor = TensorDescriptor::contiguous(dtype, shape, device);
            if byte_offset != 0 {
                descriptor = TensorDescriptor::try_strided(
    ...
    Ok((descriptor, range.end, tensor.data.cast()))
}
...
mod tests {
    use super::{
        DlpackError, DlpackExport, DlpackImport, DlpackLegacyExport, DLPACK_MAJOR, DLPACK_MINOR,
    };
    ...
    #[test]
    fn legacy_cpu_roundtrip_is_zero_copy() {
        let tensor = TensorBuffer::try_new(
            (0_u8..12).collect(),
            TensorDescriptor::contiguous(DataType::U8, vec![3, 4], Device::CPU),
        )
        .unwrap();
        let original = tensor.allocation_bytes().as_ptr();
        let allocation = tensor.shared_allocation();
        let export = DlpackLegacyExport::from_tensor(&tensor).unwrap();
        assert_eq!(allocation.strong_count(), 3);
        // SAFETY: into_raw transfers the live legacy export exactly once.
        let imported = unsafe { DlpackImport::from_legacy_raw(export.into_raw()) }.unwrap();
        assert_eq!(imported.version(), (0, 0));
        assert_eq!(imported.flags(), 0);
        let view = imported.view().unwrap();
        assert_eq!(view.descriptor().shape(), &[3, 4]);
        assert_eq!(view.allocation_bytes().as_ptr(), original);
        drop(imported);
        assert_eq!(allocation.strong_count(), 2);
    }
